...
echo $MYSQLPASSVPOP > /usr/local/src/mariadb-powermail-pass
mariadb-admin create powermail -uroot 1>/dev/null 2>/dev/null
echo "GRANT ALL PRIVILEGES ON powermail.* TO powermail@localhost IDENTIFIED BY '$MYSQLPASSVPOP'" | mariadb -uroot

#echo "CREATE USER 'powermail'@'localhost' IDENTIFIED BY '$MYSQLPASSVPOP';" | mariadb -uroot
#echo "GRANT ALL PRIVILEGES ON *.* TO 'powermail'@'localhost' WITH GRANT OPTION;" | mariadb -uroot
...
#/usr/local/src/extra-tools/ip-to-location/download-latest-ip-db.sh 1>/dev/null 2>/dev/null

## remove different mail log files and reset one used
echo > /var/log/mail.log
/bin/rm -rf /var/log/mail.info 1>/dev/null 2>/dev/null
/bin/rm -rf /var/log/mail.warn  1>/dev/null 2>/dev/null
/bin/rm -rf /var/log/mail.err 1>/dev/null 2>/dev/null


mariadb < files/powermaildb.sql
mariadb < files/powermail-extra-features.sql


sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_relay_domains_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_alias_domain_catchall_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_alias_domain_mailbox_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_alias_domain_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_alias_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_domains_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_mailbox_limit_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/postfix/sql/mysql_virtual_mailbox_maps.cf
sed -i "s/ohm8ahC2/`cat /usr/local/src/mariadb-powermail-pass`/" /etc/dovecot/dovecot-sql.conf.ext
